...
module.exports = function(){
    ...
    /*
     * idJsonArray: [{'key': , 'value': }]
     *
     *
     */
    this.removeByManyItem = function(tablename, idJsonArray, callback) {
        var sql = 'delete from ' + tablename + ' where ';
        for(var i = 0; i < idJsonArray.length ; i++) {
            if(i == idJsonArray.length - 1){
                sql += ' ' + idJsonArray[i]['key'] + ' = ' + idJsonArray[i]['value'];
            } else {
                sql += ' ' + idJsonArray[i]['key'] + ' = ' + idJsonArray[i]['value'] + ' and ';
            }
        }
	...
	function __constructor(){
		var dbConfig = lead.getConfig.get('db');
		...
		dbClient = lead.mysql.createConnection(client);
		dbClient.connect();
